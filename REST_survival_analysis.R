...
setwd("/media/adria/803c004b-f1cf-461f-8e10-ca16ad422ae3/REST_project/") 
wd <- "/media/adria/803c004b-f1cf-461f-8e10-ca16ad422ae3/REST_project/"
load(paste(wd, "Glioma_TCGA_Merged.RData", sep = ""))
...
f2<-which(mRNA_REST>1.2*med) # 1.25
f0.5<-which(mRNA_REST<0.63*med) # 0.6
mRNA_REST <- t(mRNA_REST)
mRNA_REST <- as.data.frame(mRNA_REST)
REST_bottom <- subset(mRNA_REST, mRNA_REST <= 0.6*med)
...
## Stratify patient cohort based on high and low REST mRNA expression using computed median
f2<-which(mRNA_REST>1.25*med) # 1.25
f0.5<-which(mRNA_REST<0.65*med) # 0.6
mRNA_REST <- t(mRNA_REST)
mRNA_REST <- as.data.frame(mRNA_REST)
